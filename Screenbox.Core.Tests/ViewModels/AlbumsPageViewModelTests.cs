...
public class AlbumsPageViewModelTests
{
    ...
    /// ran on, so the dispatcher queue that <see cref="AlbumsPageViewModel"/>'s constructor
    ...
    /// </summary>
    private static AlbumsPageViewModel CreateViewModel(LibraryContext libraryContext, ISettingsService settings)
    {
        DispatcherQueueTestHelper.EnsureDispatcherQueue();
        return new AlbumsPageViewModel(libraryContext, settings);
    }
